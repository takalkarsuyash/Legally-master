...
const DocumentQuery: FC = () => {
    ...
    const [asiStreamingService] = useState<ASIStreamingService>(new ASIStreamingService());
    ...
    const [documentData, setDocumentData] = useState<{ content: string; type: 'text' | 'image' } | null>(null);
    ...
    const readFileContent = (file: File): Promise<{ content: string; type: 'text' | 'image' }> => {
    ...
    const processWithASI = async (userInput: string, streamingMsg: Message): Promise<void> => {
        try {
            ...
            console.log('Starting ASI Stream with Document Data:', documentData ? 'Present' : 'NULL');
            let currentContent = '';
            ...
            try {
                ...
                for await (const chunk of asiStreamingService.streamAutoA2AFlow(
                    ...
                    // Update progress UI
                    setAgentProgress(prev => prev ? { ...prev, progress: chunk.progress } : null);
    ...
    const processWithGemini = async (userInput: string, streamingMsg: Message): Promise<void> => {
        try {
            ...
            try {
                for await (const chunk of streamingService.streamDavidAnalysis(userInput)) {
                    // Show David's analysis in real-time
                    if (chunk.content) {
                        davidContent += chunk.content;
                        setStreamingMessage(prev => prev ? { ...prev, content: davidContent } : null);
                    }
                    setAgentProgress(prev => prev ? { ...prev, progress: chunk.progress } : null);
            ...
            if (!davidResult || davidError) {
                ...
                davidResult = {
                    queryContext: {
                        ...
                        reasoning: 'I have analyzed your legal question and found relevant information.',
            ...
            try {
                for await (const chunk of streamingService.streamAndrewPresentation(davidResult, userInput)) {
                    if (chunk.content) {
                        andrewContent += chunk.content;
                        setStreamingMessage(prev => prev ? { ...prev, content: andrewContent } : null);
                    }
                    setAgentProgress(prev => prev ? { ...prev, progress: chunk.progress } : null);
    ...
    const LawyerCardComponent: FC<{ card: LawyerCard }> = ({ card }) => {
        ...
        return (
            <motion.div
                ...
                className={`p-4 rounded-xl border-2 transition-all ${isPrimary
                    ...
                    <div className="flex-1">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <div className="text-sm font-medium text-primary">
                                {lawyer.website ? (
                                    <a href={`https://${lawyer.website}`} target="_blank" rel="noopener noreferrer" className="hover:underline flex items-center gap-1">
                        ...
                        <div className="flex justify-center items-center mb-4">
                            ...
                            >
                                <motion.button
                                    ...
                                    className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 ${useASI
                                ...
                                {messages.map((message) => (
                                    ...
                                    >
                                        {message.type === 'system' ? (
                                            ...
                                            <div
                                                className={`max-w-[85%] sm:max-w-[75%] rounded-2xl p-3 sm:p-6 shadow-lg ${message.type === 'user'
                                            ...
                                            >
                                                ...
                                                {/* Lawyer Cards */}
                                                {message.lawyerCards && message.lawyerCards.length > 0 && (
                                                    <div className="mt-4 space-y-3">
                                                        <h4 className="font-semibold text-sm text-gray-700 mb-2">Recommended Lawyers:</h4>
                                                ...
                                                {message.queryContext && (
                                                    <div className="mt-3 p-3 bg-gray-50 rounded-lg">
                                                        <div className="text-xs text-gray-600">
                                                            <strong>Legal Areas:</strong> {message.queryContext.specializations?.join(', ') || 'General Legal'}
                                                            <br />
                                                            <strong>Confidence:</strong> {message.queryContext.confidence}%
                                                        </div>
                                                    </div>
                                                )}
                                        ...
                                        {agentProgress && (
                                            <div className="mt-3">
                                                <div className="flex justify-between text-xs text-gray-500 mb-1">
                                                    <span>{agentProgress.stage === 'david' ? 'Analyzing' : agentProgress.stage === 'andrew' ? 'Formatting' : 'Processing'}</span>
                                ...
                                >
                                    <div className="flex justify-between items-center">
                                        <div className="flex items-center space-x-3 sm:space-x-4">
                                            ...
                                            <div>
                                                <span className="text-xs sm:text-sm font-bold text-gray-800">{fileInfo.name}</span>
                                ...
                                >
                                    ...
                                    <label htmlFor="file-upload">
                                        <motion.button
                                            ...
                                            onClick={() => fileInputRef.current?.click()}
                                            disabled={isProcessingFile || isProcessingMessage}
                                            className={`p-2 sm:p-4 rounded-xl transition-all shadow-lg font-semibold ${isProcessingFile || isProcessingMessage
                                    ...
                                    <motion.button
                                        ...
                                        whileTap={{ scale: 0.95 }}
                                        type="submit"
                                        disabled={!inputValue.trim() || isProcessingFile || isProcessingMessage}
                                        className={`p-2 sm:p-4 rounded-xl transition-all shadow-lg font-semibold ${!inputValue.trim() || isProcessingFile || isProcessingMessage
                                            ? 'bg-gray-300 text-gray-500'
